 ...
 */
function lineToPositions(line: SunPathLine, center: Cesium.Cartesian3): Cesium.Cartesian3[] {
...
function addArrowheads(
  ...
  for (let a = 0; a < numArrows; a++) {
    ...
    const createWing = (p1: Cesium.Cartesian3, p2: Cesium.Cartesian3) =>
...
function renderLineLabels(
  ...
  const drawLineLabel = (pos: Cesium.Cartesian3, text: string, verticalPos: 'above' | 'below') => {
    addLabel(labels, pos, text, {
      color: labelColor,
      font: fontSize,
      verticalOrigin:
        verticalPos === 'above' ? Cesium.VerticalOrigin.BOTTOM : Cesium.VerticalOrigin.TOP,
  ...
  if (['month', 'solstice', 'dst'].includes(line.type)) {
    // Main label (mid)
    const midPos = line.midLabelPoint
      ? toGlobal(line.midLabelPoint)
...
export function useSunPathPrimitives(
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      /* location has no DST or lookup failed */
    }
    ...
    try {
      ...
      if (sunPos.altitude > -0.2) {
        ...
        billboardCollection.add({
          position: sunCart,
          image:
            'data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMzIiIGhlaWdodD0iMzIiIHZpZXdCb3g9IjAgMCAzMiAzMiIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48Y2lyY2xlIGN4PSIxNiIgY3k9IjE2IiByPSIxMiIgZmlsbD0iI0ZGRkYwMCIgc3Ryb2tlPSIjRkZBNTAwIiBzdHJva2Utd2lkdGg9IjQiLz48L3N2Zz4=', // Yellow circle with orange stroke
